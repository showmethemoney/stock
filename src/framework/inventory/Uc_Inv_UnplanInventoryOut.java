...
public class Uc_Inv_UnplanInventoryOut extends AbstractUcCore {
	protected static final Logger logger = LoggerFactory.getLogger(Uc_Inv_UnplanInventoryOut.class);

	public String crtUnplanInventoryOut(TWStockPrice stock, int quantity) {
		String result = null;
		...
		try {
			...
			dataParameters.put("REFERENCEUNITPRICE",
					new BigDecimal(Math.floor(Double.parseDouble(stock.getLastPrice())))); // 參考單價
			...
			Map response = service.request(Uc_Core_Constant.NAMED_SERVICE_UC_INV_UNPLANINVENTORYOUT,
					Uc_Core_Constant.NAMED_METHOD_CRTUNPLANINVENTORYOUT, login(), parameters);

			logger.info("RETURNVALUE: {}, RETURNMSG: {}", response.get("RETURNVALUE"), response.get("RETURNMSG"));
			logger.info("SITEID: {}, UONO: {}, STATUS: {}", response.get("SITEID"), response.get("UONO"),
					response.get("STATUS"));
	...
	public String confirm(String uono) {
		String result = null;
		...
		try {
			...
			Map response = service.request(Uc_Core_Constant.NAMED_SERVICE_UC_INV_UNPLANINVENTORYOUT,
					Uc_Core_Constant.NAMED_METHOD_CONFIRM, login(), parameters);
