...
public class TWStockService {
	...
	public List<TWStockDailyTrading> getDailyTrade(String type, String stock, Date start, Date end) {
		...
		try {
			DailyTradingService dailyTradingService = StockType.TSE.getType().equalsIgnoreCase(type)
					? new TSEDailyTradingService()
					: new OTCDailyTradingService();
	...
	public List<TWStock> getIndustryStocksByType(String type, String industry) {
		...
		try {
			...
			// String response =
			// restTemplate.getForEntity(String.format(URL_GET_STOCKS_BY_INDUSTRY, type,
			// industry), String.class).getBody();

			String response = restfulClient(String.format(URL_GET_STOCKS_BY_INDUSTRY, type, industry));
	...
	public List<TWIndustry> getIndustry(String type) {
		...
		try {
			...
			// restTemplate.getForEntity(URL_GET_INDUSTRY, String.class);

			// {"tse":[{"name":"封閉式基金","code":"A0"}],"otc":[{"name":"ETF","code":"B0"}]}
			// String response = responseEntity.getBody();
			String response = restfulClient(URL_GET_INDUSTRY);
	...
	public TWStockPrice getTodayStockPrice(String stock) {
		...
		try {
			...
			TWStockPriceInfo stockPriceInfo = objectMapper.readValue(
					StringUtils.trim(IOUtils.toString(response.getEntity().getContent(), "UTF-8")),
					TWStockPriceInfo.class);
	...
	protected String restfulClient(String url) {
		String result = null;
