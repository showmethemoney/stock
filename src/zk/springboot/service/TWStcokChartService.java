...
public class TWStcokChartService {
	...
	public SimpleHiLoModel getCandlestickChartModel(List<TWStockDailyTrading> twStockDailyTradings) {
		...
		try {
			...
			DateTimeFormatter df = new DateTimeFormatterBuilder().parseLenient().appendPattern("yy/MM/dd").toFormatter()
			...
			while (iterator.hasNext()) {
				...
				result.addValue(Date.from(LocalDate.from(localDate).atStartOfDay(ZoneId.of("UTC+8")).toInstant()),
						Double.parseDouble(trading.getOpeningPrice()), Double.parseDouble(trading.getDayHigh()),
						Double.parseDouble(trading.getDayLow()), Double.parseDouble(trading.getClosingPrice()),
